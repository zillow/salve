...
def export_horizonnet_zind_predictions(
    ...
    for row in tsv_rows:
        ...
        for pano_guid in pano_guids:
            ...
            render_on_pano = np.random.rand() < 0.02 # True True # True # False # 
            if render_on_pano:
                plt.imshow(img_resized)
                pred_obj.render_layout_on_pano(img_h, img_w)
                plt.title(f"Pano {i} from Building {zind_building_id}")
                plt.tight_layout()
                os.makedirs(f"prod_pred_model_visualizations_2022_08_12_bridge/{model_name}_bev", exist_ok=True)
                plt.savefig(
                    f"prod_pred_model_visualizations_2022_08_12_bridge/{model_name}_bev/{zind_building_id}_{i}.jpg",
                    dpi=400,
                )
                # plt.show()
                plt.close("all")
...
if __name__ == "__main__":
    ...
    zind_building_ids = DATASET_SPLITS["test"]#"val"] #"train"] #] # ] #  ] # ""train"] #
    #import pdb; pdb.set_trace()

...
def prune_to_2d_bbox(pts: np.ndarray, rgb: np.ndarray, xmin: float, ymin: float, xmax: float, ymax: float) -> np.ndarray:
    """"""
    x = pts[:, 0]
    y = pts[:, 1]
...
def interp_dense_grid_from_sparse(
    ...
    interp_rgb_vals = griddata(points=points[:,:2], values=rgb_values, xi=grid_coords, method='nearest' if is_semantics else 'linear') #) # # or method='linear', method='cubic'

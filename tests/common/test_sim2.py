"""Unit tests on the Similarity(2) class.

Under MIT license from:
https://github.com/argoai/argoverse-api/blob/master/tests/test_sim2.py
"""
...
def test_transform_point_cloud() -> None:
    ...
    for sample in range(1000):
        ...
        pts_a = aSb.transform_point_cloud(copy.deepcopy(pts_b))

        assert isinstance(pts_a, np.ndarray)
        assert pts_a.shape == (25, 2)
...
def test_save_as_json() -> None:
    ...
    with tempfile.NamedTemporaryFile(suffix="b_Sim2_c.json") as save_fpath:
        bSc.save_as_json(save_fpath=save_fpath.name)

        bSc_dict = io_utils.read_json_file(save_fpath.name)

"""Utilities for computing running averages.

Adapted from `mseg-semantic` (MIT License):
...
class SegmentationAverageMeter(AverageMeter):
    ...
    def update_metrics_cpu(self, pred: np.ndarray, target: np.ndarray, num_classes: int) -> None:
        """Updates metrics on the CPU using new samples.

        Args:
            pred: numpy array of any shape representing prediction.
            target: numpy array of any shape represeting ground truth labels.
    ...
    def update_metrics_gpu(
        ...
        """Updates metrics on the GPU using new samples.

        Args:
            pred: pytorch tensor of any shape representing prediction.
            target: pytorch tensor of any shape represeting ground truth labels.
            num_classes: number of classes.
            ignore_idx:
        """
...
def exclusion(array: np.ndarray, excluded_ids: List[int]) -> np.ndarray:
    """Takes in array of IoU/Acc., return non-excluded IoU/acc values"""

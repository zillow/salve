...
def computeUVN_vec(n, in_, planeID):
    ...
    Args:
        n: array of shape (N,3)
        in_: array of shape (MN, 1)
        planeID: array of shape (N,)
...
def warpImageFast(im, XXdense, YYdense):
    ...
    im = im[int(round(minY - 1)) : int(round(maxY)), int(round(minX - 1)) : int(round(maxX))]
    ...
    im_warp = np.stack(
        [map_coordinates(im[..., c], coordinates, order=1).reshape(out_shape) for c in range(im.shape[-1])], axis=-1
    )
...
def combineEdgesN(edges):
    ...
    for i in range(numLine):
        ...
        ori_lines[i, 6] = np.arccos(
            (np.dot(coord1, coord2) / (np.linalg.norm(coord1) * np.linalg.norm(coord2))).clip(-1, 1)
        )
        ...
        for i in range(len(checkIDs2)):
            ...
            for k in range(len(checkIDs3)):
                ...
                if vote3 > voteMax:
                    ...
                    if voteMax != 0:
                        tmp = (
                            voteBinPoints[[checkID1Max, checkID2Max, checkID3Max]]
...
def paintParameterLine(parameterLine, width, height):
    ...
    for i in range(len(lines)):
        ...
        xyz = uv2xyzN(np.hstack([u, v]), lines[i, 3])
        uv = xyz2uvN(xyz, 1)
        m = np.minimum(np.floor((uv[:, 0] + np.pi) / (2 * np.pi) * width) + 1, width).astype(np.int32)
        n = np.minimum(np.floor(((np.pi / 2) - uv[:, 1]) / np.pi * height) + 1, height).astype(np.int32)
        panoEdgeC[n - 1, m - 1] = i
...
def panoEdgeDetection(img, viewSize=320, qError=0.7, refineIter=3):
    ...
    y = np.concatenate([yh, yp, [np.pi / 2.0, -np.pi / 2]])

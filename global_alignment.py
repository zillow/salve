...
def global_averaging(i2Ri1_dict) -> List[Optional[Rot3]]:
	"""Lift 2d rotation matrix to 3d, and then convert back to 2d"""
	i2Ri1_dict_Rot3 = {}
...
def greedily_construct_mst(i2Ri1_dict: Dict[Tuple[int,int], Rot3]) -> List[Rot3]:
...
def test_greedily_construct_mst():
	...
	for i1 in range(4):
		i2 = i1 + 1
		i2Ri1_dict[(i1,i2)] = wRi_list_gt[i2].between(wRi_list_gt[i1])

...
We'll now show an example for a specific home, Building 0000, Floor 01. We can see that a transformation (R,t) followed by a reflection over the y-axis is equivalent to a transformation by (R^T,-t) followed by no reflection.
...
Consider the red inset circles on the left and right. We can see that the rotation angle must be inverted in order for the panorama orientation to stay correct (facing towards a window).
